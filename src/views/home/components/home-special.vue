  ...
  <HomePanel title="最新专题">
    ...
    <div class="special-list" ref="target">
      <div class="special-item" v-for="item in list" :key="item.id">
        ...
        <div class="foot">
          <span class="like"><el-icon><Star /></el-icon>{{ item.collectNum }}</span>
          <span class="view"><el-icon><View /></el-icon>{{  item.viewNum }}</span>
          <span class="reply"><el-icon><ChatLineSquare /></el-icon>{{ item.replyNum }}</span>
        </div>
      </div>
    </div>
  </HomePanel>
